...
grossd(8) reads configuration data from F<%{sysconfdir}/etc/grossd.conf>
(or the file specified with B<-f> on the command line). 
An example configuration file is installed by default. You have to
set some configuration options in order to get grossd(8) running in 
...
is the address grossd(8) listens for status queries. Default is C<localhost>.

=item B<status_port>

is the port number grossd(8) listens for status queries. Default is
C<5522>.

=item B<protocol>

activates the server protocols grossd(8) will support. Valid settings are 
...
B<query_timelimit> and/or B<pool_maxthreads>.  If you run grossd(8) in a
...
grossd(8) will update the database only if response is
C<STATUS_GREY>.  Setting it to C<always> may reduce the impact on
DNS servers. 

=item B<grey_mask>

is the mask for grossd(8) to use when matching
C<smtp-client-ip> against the database. Default is 24, which makes grossd(8)
to treat addresses like C<a.b.c.d> as C<a.b.c.0>.
Setting B<grey_mask> to 32 makes grossd(8) to require that consecutive
...
is the full path of the file grossd(8) writes its pid into.
You can set parameter C<check>, if you want to keep grossd(8) from
...
grossd(8) to be able to handle per second, and multiply that with
...
is the threshold after which grossd(8) sends 
...
greater than B<block_threshold> grossd(8) sends a
...
is analogous to B<block_threshold>, except at the threshold grossd(8)
...
any checks grossd(8) will act as a traditional greylisting server.
...
block lists take into account that grossd(8) makes all queries as
...
I<definitive> check, that is grossd(8) waits for the check to complete
...
option. Highly recommended if you use grossd(8) as a traditional greylister.
...
is the mapping result template grossd(8) uses for a C<STATUS_GREY>
result. Default is C<$X4.4.3|$NPlease$ try$ again$ later>.

=item B<sjsms_response_match>

is the mapping result template grossd(8) uses for a C<STATUS_MATCH>
result. Default is C<$Y>.

=item B<sjsms_response_trust>

is the mapping result template grossd(8) uses for a
...
B<SJSMS> to query grossd(8). It's also a good idea to exclude
L<postmaster> and L<abuse> addresses before querying grossd(8).
...
grossd server address at the C<smtpd_recipient_restrictions>
...
Exim can be configured to query grossd(8) via
